...
const StaffSelfAttendanceReport = () => {
  ...
  // Adjusted to match common Next.js Auth context patterns
  ...
  // Determine days in month for calendar rendering
    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen font-sans">
      <div className="max-w-5xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <button
            onClick={exportPDF}
            className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-bold shadow hover:bg-indigo-700 transition-colors w-full sm:w-auto"
          >
            📄 Export PDF
          </button>
        </div>
          ...
          {loading ? (
            ...
            monthlyRecords.map((record) => (
              <div key={record.id} className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                ...
                <div className={`px-3 sm:px-5 py-1 sm:py-2 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest border ${
        ...
        {/* CALENDAR VIEW */}
        <div className="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-gray-100 mt-6">
          <h3 className="font-black text-gray-700 mb-4 uppercase text-sm sm:text-base">Monthly Calendar</h3>
          <div className="grid grid-cols-7 gap-1 sm:gap-2 text-center text-[8px] sm:text-xs font-bold">
            {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d => (
              <div key={d} className="text-gray-400 pb-2">{d}</div>
            ))}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  key={dateStr}
                  className={`h-8 sm:h-12 flex items-center justify-center rounded-xl border text-[10px] sm:text-sm transition-all
